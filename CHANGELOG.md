...
### Added
- **Esecuzione dell'autopricer a richiesta, che prosegue in background**: pulsante "Esegui adesso" nella scheda Regole. `POST /api/pricing/run` risponde `202` e restituisce subito il controllo invece di restare aperto per ore — una esecuzione vera dura ore, perché il limite verso Card Trader è di 20 richieste al minuto. Nuovi `GET /api/pricing/run/current` (avanzamento) e `POST /api/pricing/run/cancel` (interruzione, che avviene fra una carta e la successiva). Un indicatore in barra di stato mostra l'avanzamento da qualunque pagina, quindi si può lanciare il ricalcolo e andare a lavorare altrove; sopravvive a un ricaricamento del browser, perché i contatori stanno su `PricingRunLog`. Nessuna migration
- **Filtri sull'anteprima dei prezzi**: `POST /api/pricing/preview` accetta `minPrice`, `maxPrice` ed `expansionId` oltre al numero di carte. Prima campionava sempre e solo le carte di maggior valore, quindi una modifica alla regola del bulk restava invisibile
- **Applicazione selettiva dall'anteprima** (`POST /api/pricing/apply`): caselle di selezione nella griglia dei calcoli e pulsante "Applica le selezionate", che riprezza davvero le carte spuntate **anche con il profilo in dry-run**. È il modo di uscire dalla simulazione un pezzo alla volta, senza attivare la scrittura anche sull'esecuzione notturna. I prezzi calcolati dall'anteprima non vengono rimandati al server — arriverebbero dal browser e l'API non deve fidarsene: le carte sono rivalutate su dati di mercato freschi un attimo prima di scrivere
...
### Changed
- **Una sola esecuzione dell'autopricer alla volta**: nuovo `PricingRunCoordinator` (singleton) da cui passano notturna, esecuzione manuale e applicazione dall'anteprima. Il limite di 20 richieste al minuto verso Card Trader è condiviso da tutta l'applicazione: due esecuzioni in parallelo non vanno il doppio più veloci, si dimezzano a vicenda e rallentano anche le sincronizzazioni. Se una manuale è ancora in corso alle 03:30, la notturna lo registra a log e salta invece di sovrapporsi; una richiesta rifiutata riceve `409` con il nome dell'esecuzione che occupa il posto
...
### Fixed
- **L'applicazione dei prezzi non scriveva niente pur risultando conclusa**: la modalità dry-run si decide in due punti, non uno. `AutoPricingService.RunAsync` calcola se scrivere, ma `PricingEngine` consulta `profile.DryRun` per conto proprio e restituisce già `SimulatedDryRun`; forzare solo il primo lasciava l'esito invariato. Aggiunto il ramo simmetrico che riporta quell'esito ad `Applied`. Trovato da un test, non a video — a schermo l'esecuzione risultava terminata correttamente. `forceApply` non prevale comunque mai su `forceDryRun`, così l'anteprima resta innocua per costruzione
- **Il salvataggio del profilo di pricing falliva appena si toccava una regola**: l'API scriveva `referenceMode` come stringa ma non era configurata per rileggerlo, quindi il corpo rimandato indietro dalla maschera non superava il binding e tornava un 400. Registrato `JsonStringEnumConverter`; la configurazione JSON vive ora in `ApiJsonOptions`, coperta da un test che parte dal payload vero della maschera
...
### Added
- **Storico dei prezzi** (`PriceHistoryEntries`): serie temporale di prezzo e quantità per ogni inserzione, alimentata dalla sincronizzazione notturna, che scarica già l'export completo e quindi non consuma chiamate API aggiuntive. Registra solo i valori che cambiano rispetto alla rilevazione precedente, più un primo punto per ogni inserzione: un magazzino di 35.000 carte scritto per intero ogni notte produrrebbe circa 12 milioni di righe l'anno per rappresentare in gran parte prezzi fermi. Cattura anche i cambi manuali e quelli dell'autopricer nativo, che `PriceChangeLogs` non vede

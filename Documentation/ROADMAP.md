...
| 2026-09-05 | Feature — **Grafico storico prezzi in "Nuovo Prodotto"**: una linea per inserzione (condizione/lingua/foil), alimentato da `PriceHistoryEntries`. Aggiunti anche link "Vedi su Card Trader", nome italiano e numero di raccolta sotto l'immagine, colonna CT nella griglia "Coda inserzioni" |
...
- [x] ~~**Grafici di andamento prezzi**~~ — fatto il 2026-09-05, nella pagina "Nuovo Prodotto" invece che in una scheda dedicata: nuovo endpoint `GET /api/cardtrader/blueprints/{id}/price-history`, grafico Chart.js con una linea per inserzione (condizione/lingua/foil possono differire). Insieme, aggiunti anche il link "Vedi su Card Trader" e il nome italiano/numero di raccolta sotto l'immagine, e una colonna CT nella griglia "Coda inserzioni". Non ancora deployato, non verificato dal vivo nel browser (nessuna credenziale in sessione)
- [ ] **Affiancare il riferimento di mercato al grafico storico**: oggi il grafico in "Nuovo Prodotto" mostra solo i prezzi delle mie inserzioni (`PriceHistoryEntries`). Affiancarci il riferimento di mercato calcolato dall'autopricer (`PriceChangeLogs.ReferencePrice`) darebbe il contesto per capire se un prezzo era corretto quando è stato scritto — utile anche per il punto sotto sulla scala di `ReferencePrice`

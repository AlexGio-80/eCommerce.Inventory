...
- [x] ~~**Forzare il prezzo proposto sulle carte bloccate dal guardrail, dalla scheda Storico**~~ — fatto il 2026-09-05: nuovo parametro `bypassGuardrail` che percorre `PricingEngine.Evaluate` → `AutoPricingService.RunAsync` → `PricingRunCoordinator` → `POST /api/pricing/apply` (stesso endpoint dell'anteprima, con `BypassGuardrail: true` sul corpo). Nella scheda Storico la griglia dei calcoli mostra la casella di selezione solo sulle righe con esito `BlockedByGuardrail` — senza «seleziona tutto», un bypass va scelto carta per carta — e il pulsante "Applica comunque" rivaluta su dati freschi come «Applica le selezionate». Non testato dal vivo nel browser (serve login); verificato con build Angular pulita e 4 nuovi test dedicati (motore + servizio)

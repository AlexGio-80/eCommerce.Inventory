...
- **Firma obbligatoria sul webhook Card Trader (2026-09-05)**: `X-Signature` mancante o non valida restituisce `401` invece di processare l'evento con solo un warning. Corretto anche l'ordine fra `EnableBuffering()` e il model binding di `[FromBody]` — il buffering va abilitato prima, non dentro l'azione, altrimenti la rilettura del corpo per la verifica HMAC ottiene sempre una stringa vuota — con un resource filter dedicato (`EnableRequestBodyBufferingAttribute`) e test HTTP end-to-end (`TestServer`) che coprono header mancante, firma non valida e firma valida.

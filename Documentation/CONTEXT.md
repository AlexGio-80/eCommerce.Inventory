...
- **"Applica comunque" per le carte bloccate dal guardrail (2026-09-05)**: dalla griglia dei calcoli in Storico, le sole righe con esito `BlockedByGuardrail` mostrano una casella di selezione (niente "seleziona tutto": è un bypass, va scelto carta per carta). Il pulsante "Applica comunque" rivaluta quelle carte su dati freschi e scrive su Card Trader anche se il limite di variazione massima scatterebbe ancora — stesso endpoint `POST /api/pricing/apply` dell'"Applica le selezionate" dall'anteprima, con un flag `bypassGuardrail` in più che percorre tutta la catena (`PricingEngine.Evaluate` → `AutoPricingService` → `PricingRunCoordinator`)
- **Filtro per Origine nella scheda Esecuzioni (2026-09-05)**: menù a tendina per restringere lo storico a Notturna/Vendita/Manuale/Nuova inserzione, e limite di righe caricate salito da 20 a 50. Senza, una giornata con molte carte pubblicate faceva sparire la notturna di quella notte dietro alle "Nuova inserzione", senza modo di ritrovarla
- **Orario della notturna dell'autopricer**: `AutoPricing:RunTime` in appsettings (default e valore attuale `03:30`, ora del server). Si cambia a mano nel file di configurazione — non c'è un endpoint o un controllo in interfaccia — e serve un riavvio del servizio Windows per applicarlo

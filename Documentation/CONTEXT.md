...
**Ultimo aggiornamento:** 2026-09-25 (sessione 17 — trovato e corretto perché il bulk non si vendeva più: sovrapprezzo di Card Trader scartato sotto 0,25 € e storico prezzi che non registrava i riprezzi dell'autopricer; pubblicato, 26.011 inserzioni riportate a 0,05 €, da verificare la prima notturna. Sessione 16 — deployate e verificate in produzione: firma obbligatoria sul webhook Card Trader, "Applica comunque" per bypassare il guardrail dalla scheda Storico, filtro per Origine nella scheda Esecuzioni, grafico storico prezzi + riferimento di mercato + link Card Trader + nome italiano/numero di raccolta nella pagina "Nuovo Prodotto" (grafico poi spostato nella colonna "Le mie inserzioni" su richiesta). Deciso anche che l'autopricer è uscito dal dry-run (gira in modalità reale da alcune notti). Riserva aperta dell'utente: su carte con molte inserzioni diverse la colonna col grafico risulta un po' affollata, da tenere d'occhio. Sessione 15: filtro "solo venditori Cardtrader Zero" nell'autopricer, non ancora deployato. Sessione 14: autopricer lanciabile a mano durante il giorno, con esecuzione in background; anteprima filtrabile per fascia ed espansione; applicazione delle sole carte selezionate. Sessione 13: riprezzo delle carte appena pubblicate, collegamento a Card Trader nelle griglie, fix salvataggio del profilo. Sessione 12: sicurezza, API chiusa per difetto)
**Fase:** In produzione (uso quotidiano attivo)

> **Sessione 17 (2026-09-25), pubblicata e verificata in produzione il 26/09; ripristino eseguito (26.011 inserzioni a 0,05 € confermate dall'export CT):** trovato perché il bulk non si vendeva più.
> Il motore scartava come implausibile il sovrapprezzo di Card Trader sotto 0,25 € (0,09 € fissi,
> rapporto fino a 1,9, oltre il limite di 1,15) e prezzava ogni carta circa 0,09 € sopra la posizione
> configurata: 26.547 inserzioni a 0,05 € del 30/08 alzate, vendite ≤0,25 € da ~500–1.000 a ~60–130
> copie a settimana. Ora il sovrapprezzo si sottrae come differenza. Nella stessa indagine: lo storico
> prezzi non registrava i riprezzi dell'autopricer (confrontava con `InventoryItem`, già aggiornato).
> **Verificato il 26/09 sulla prima notturna**: conversione riuscita su tutte le 3.459 carte sotto 0,25 €, e 2.918 delle 2.925 carte a 0,05 € valutate restano lì (le 6 salite sono rialzi di mercato corretti). Storico prezzi verificato lo stesso giorno con una sincronizzazione manuale del solo inventario (quella notturna era stata interrotta dal riavvio per la pubblicazione delle 07:13): **8.373 rilevazioni** contro le 4–40 delle notti precedenti, recupero dei cambi accumulati dal 30/08.
> Dalla stessa verifica, **pubblicata il 26/09** (campo verificato in interfaccia, in produzione a 0,15 €; effetto da vedere nelle notturne): soglia in euro sotto la quale il guardrail percentuale non scatta (`PricingProfile.GuardrailExemptAmount`, default 0,10 €): 226 carte di bulk erano ferme al −50%, che non applica ribassi parziali. Vedi CHANGELOG e ROADMAP.
...
- **Storico dei prezzi (2026-08-29)**: tabella `PriceHistoryEntries` alimentata dalla sincronizzazione notturna, che scarica gia' l'export completo e quindi non consuma chiamate API. Registra prezzo e quantita' solo quando cambiano, piu' un primo punto per ogni inserzione. **Fino al 2026-09-25 non registrava i riprezzi dell'autopricer** (confrontava con l'`InventoryItem`, già aggiornato da chi scrive su Card Trader): le serie fra il 30/08 e il 25/09 sono quasi piatte. Dal 2026-09-05 alimenta il grafico nella pagina "Nuovo Prodotto" (vedi sotto)
- **Grafico storico prezzi, link Card Trader e nome italiano in "Nuovo Prodotto" (2026-09-05)**: sotto la maschera di inserimento, un grafico Chart.js con una linea per inserzione della carta selezionata (condizione/lingua/foil possono differire), alimentato da `PriceHistoryEntries` via il nuovo `GET /api/cardtrader/blueprints/{id}/price-history`. Le serie sono a delta e non allineate: si uniscono tutte le date in un'unica scaletta ordinata e ogni serie vi si proietta con un vuoto dove manca la rilevazione (`spanGaps`). Aggiunti anche: link "Vedi su Card Trader" e nome italiano/numero di raccolta sotto l'immagine, colonna CT nella griglia "Coda inserzioni". Il componente è standalone e lazy-loaded, quindi registra Chart.js (`Chart.register(...registerables)`) nel proprio file invece di affidarsi al costruttore di `ReportingModule`. **Verificato in produzione il 2026-09-05**
- **Riferimento di mercato affiancato al grafico storico prezzi (2026-09-05)**: stessa pagina, linea tratteggiata "Riferimento di mercato" accanto alle mie inserzioni. Nuovo campo `PriceChangeLog.ReferenceSellerPrice` (migration `20260905092416_AddReferenceSellerPriceToPriceChangeLogs`): `PricingEngine.Evaluate` riporta già `ReferencePrice` (scala vetrina) alla scala venditore togliendo il sovrapprezzo di Card Trader (dal 2026-09-25 per differenza, prima per rapporto), così è comparabile con `OldPrice`/`ProposedPrice` e con `PriceHistoryEntries.Price` sullo stesso asse — `ReferencePrice` grezzo da solo sarebbe apparso sfalsato pur non essendo un errore. Null sulle valutazioni scritte prima che il campo esistesse. **Verificato in produzione il 2026-09-05**
- **Grafico storico prezzi spostato nella colonna "Le mie inserzioni" (2026-09-05)**: su richiesta dopo la prima verifica, per vederlo senza scorrere la pagina — colonna allargata da 250 a 340px, grafico sotto la legenda dei badge. **Riserva aperta dell'utente**: su carte con molte inserzioni diverse (condizioni/lingue/foil differenti) la colonna risulta un po' affollata; nessuna modifica ulteriore programmata, da valutare con l'uso
- **Autopricer ritarato (2026-08-29)**: il motore confrontava il prezzo venditore con i prezzi acquirente del marketplace, e si posizionava per numero d'ordine su mercati profondi da 3 a 29 offerte. Ora converte fra le due scale ricavando il sovrapprezzo dalla propria inserzione nel feed (dal 2026-09-25 come differenza da sottrarre: il rapporto con limite 1,15 falliva su tutto il bulk), si colloca per percentile, non può mai finire sull'offerta più cara, e ha un guardrail asimmetrico (+300% / −25%)
- **La vendita scala subito la giacenza (2026-08-29)**: il webhook `order.create` sottrae le quantità vendute, con guardia sui webhook duplicati. Prima l'inventario mostrava carte già vendute fino alla notte. **Nota (2026-09-05)**: il webhook non è mai stato raggiungibile dall'esterno (vedi sotto), quindi finora questo percorso non si è mai attivato su un ordine reale — a tenere l'inventario allineato è il giro a polling. Resta comunque il fallback corretto per quando/se il webhook verrà esposto
...
- **"Applica comunque" per le carte bloccate dal guardrail (2026-09-05)**: dalla griglia dei calcoli in Storico, le sole righe con esito `BlockedByGuardrail` mostrano una casella di selezione (niente "seleziona tutto": è un bypass, va scelto carta per carta). Il pulsante "Applica comunque" rivaluta quelle carte su dati freschi e scrive su Card Trader anche se il limite di variazione massima scatterebbe ancora — stesso endpoint `POST /api/pricing/apply` dell'"Applica le selezionate" dall'anteprima, con un flag `bypassGuardrail` in più che percorre tutta la catena (`PricingEngine.Evaluate` → `AutoPricingService` → `PricingRunCoordinator`). **Verificato in produzione il 2026-09-05**: forza correttamente il prezzo su una carta bloccata dal guardrail
- **Filtro per Origine nella scheda Esecuzioni (2026-09-05)**: menù a tendina per restringere lo storico a Notturna/Vendita/Manuale/Nuova inserzione, e limite di righe caricate salito da 20 a 50. Senza, una giornata con molte carte pubblicate faceva sparire la notturna di quella notte dietro alle "Nuova inserzione", senza modo di ritrovarla. **Verificato in produzione il 2026-09-05**
- **Orario della notturna dell'autopricer**: `AutoPricing:RunTime` in appsettings (valore attuale `01:00`, ora del server — cambiato dal default `03:30` il 2026-09-05). Si cambia a mano nel file di configurazione — non c'è un endpoint o un controllo in interfaccia — e serve un riavvio del servizio Windows per applicarlo
- **Autopricer lanciabile a mano, che gira in background (2026-09-02)**: pulsante "Esegui adesso" nella scheda Regole. L'esecuzione prosegue sul server, quindi si può cambiare pagina; l'avanzamento resta in un indicatore nella barra in alto, visibile da ogni pagina e con un clic per tornare all'autopricer. Sopravvive a un ricaricamento del browser, perché i contatori stanno su `PricingRunLog`. Ne gira **una sola alla volta** — vale anche per la notturna, che passa dallo stesso coordinatore — perché il limite di 20 richieste al minuto verso Card Trader è condiviso. C'è il pulsante per interrompere: l'arresto avviene fra una carta e la successiva
- **Anteprima filtrabile e applicabile (2026-09-02)**: l'anteprima si restringe per fascia di prezzo e per espansione, così una regola si prova sulle carte che governa invece che sempre sulle più care. Nella griglia ci sono le caselle di selezione e il pulsante «Applica le selezionate», che riprezza davvero le carte spuntate **anche con il profilo in dry-run**: è la strada per uscire dalla simulazione un pezzo alla volta. Le carte vengono rivalutate su dati di mercato freschi prima di scrivere, quindi il prezzo applicato può differire di poco da quello visto a schermo
- **Filtro "solo venditori Cardtrader Zero" nell'autopricer (2026-09-04)**: nuovo campo `IncludeOnlyCtZeroSellers` su `PricingProfile`, filtra le offerte comparabili su `user.can_sell_via_hub` (il campo con cui l'API segnala i venditori Cardtrader Zero — passano dal magazzino/controllo qualità di Card Trader). Serve a non prendere a riferimento venditori nuovi o casuali con prezzi fuori mercato. Checkbox in "Venditori di riferimento". Il filtro per area geografica richiesto insieme (europei/americani/canadesi/...) non serve codice nuovo: il campo "Paesi ammessi" già esistente (CSV di codici ISO) copre il caso, perché l'API non espone categorie regionali
...
- **Firma obbligatoria sul webhook Card Trader (2026-09-05)**: header `Signature` mancante o non valido restituisce `401` invece di processare l'evento con solo un warning. Corretto anche l'ordine fra `EnableBuffering()` e il model binding di `[FromBody]` — il buffering va abilitato prima, non dentro l'azione, altrimenti la rilettura del corpo per la verifica HMAC ottiene sempre una stringa vuota — con un resource filter dedicato (`EnableRequestBodyBufferingAttribute`) e test HTTP end-to-end (`TestServer`) che coprono header mancante, firma non valida e firma valida. Verificando l'instradamento esterno (punto in sospeso) sono emersi altri due bug indipendenti, ora corretti: il controller cercava l'header `X-Signature` invece del vero `Signature` (documentazione ufficiale Card Trader), e lo shared secret di produzione era ancora un placeholder — il valore reale si ottiene da `GET /info` con il Bearer token, non dal pannello web.
...
- **Punti di sicurezza — tutti chiusi il 2026-08-29**, codice in produzione e operazioni sul database eseguite:
  ...
  - **Verificato il 2026-09-05**: il webhook non era instradato dall'esterno per niente. Nessun `webhook_url` mai registrato su Card Trader, e comunque non ci sarebbe stato un percorso funzionante fino alla macchina (Kestrel solo su loopback, IIS senza reverse proxy verso `:5152`). **Messo in stand-by**: la macchina è dietro un router di casa, e il giro con ricezione ordini a polling tiene comunque l'inventario allineato — non vale il rischio di aprire una porta per un webhook opzionale (vedi ROADMAP.md)
...
|------|-----------|--------|
| 2026-09-25 | Il sovrapprezzo di Card Trader si ricava come differenza e si sottrae, non come rapporto | È un importo a scaglioni (0,09 € fino a 0,25 €, 0,10 € fino a circa 5 €, poi decine di centesimi), misurato su 1.089 coppie reali. Sul bulk il rapporto arriva a 1,9 e il limite di 1,15 scartava tutte le carte sotto 0,25 €, prezzandole circa 0,09 € sopra la posizione configurata. Se non ricavabile si sottrae il minimo noto (0,09 €) |
| 2026-09-25 | Lo storico prezzi confronta con l'ultima rilevazione salvata, non con l'`InventoryItem` | L'autopricer aggiorna `ListingPrice` quando scrive su Card Trader: alla sincronizzazione i due valori coincidevano e i suoi riprezzi non venivano mai registrati |
| 2026-08-29 | Il sovrapprezzo fra prezzo esposto e prezzo incassato si ricava dalla propria inserzione nel feed | Il sovrapprezzo di Card Trader non è documentato. Ricavarlo dalla propria offerta lo rende esatto per definizione e autoaggiornante, senza reverse engineering. (La forma a rapporto scelta allora è stata sostituita il 2026-09-25, vedi sopra) |
...
- **Nuova migration `20260904170854_AddIncludeOnlyCtZeroSellers`**: aggiunge `IncludeOnlyCtZeroSellers bit NOT NULL DEFAULT 0` a `PricingProfiles`. Generata con `dotnet ef migrations add` (serve `--context ApplicationDbContext`, il progetto ne ha due per via di Health Checks UI). Si applica da sola all'avvio come le altre.
- **Nuova migration `20260926050752_AddGuardrailExemptAmount`**: aggiunge `GuardrailExemptAmount decimal(9,2) NOT NULL DEFAULT 0.10` a `PricingProfiles`. Default scritto a mano nella migration (EF generava 0, cioè soglia disattivata sul profilo esistente). Si applica da sola all'avvio.
- **Nuova migration `20260905092416_AddReferenceSellerPriceToPriceChangeLogs`**: aggiunge `ReferenceSellerPrice decimal(18,2) NULL` a `PriceChangeLogs` — il riferimento di mercato riportato alla scala venditore, comparabile con `OldPrice`/`ProposedPrice` e con `PriceHistoryEntries.Price`. Null sulle righe scritte prima che il campo esistesse: il grafico storico prezzi in "Nuovo Prodotto" le esclude. Si applica da sola all'avvio come le altre.
...
- **⚠️ «Applica» dall'anteprima scavalca il dry-run**: è l'unica strada che scrive su Card Trader pur avendo il profilo in simulazione, ed è voluta — serve ad applicare su carte appena guardate una per una. Il registro lo segnala con un warning esplicito. Da tenere presente che l'anteprima invece non scrive **mai**: `forceApply` non prevale su `forceDryRun`
- **⚠️ Il dry-run si decide in due punti**: `AutoPricingService.RunAsync` calcola la modalità, ma `PricingEngine` guarda `profile.DryRun` per conto suo e restituisce già `SimulatedDryRun`. Chi tocca la modalità deve considerarli entrambi: il primo tentativo di `forceApply` ne cambiava uno solo e non scriveva nulla
- **Una sola esecuzione dell'autopricer alla volta**: `PricingRunCoordinator` è singleton e fa da collo di bottiglia per notturna, esecuzione manuale e applicazione dall'anteprima. Non è prudenza generica — il limite di 20 richieste al minuto verso Card Trader è condiviso da tutta l'applicazione, quindi due esecuzioni in parallelo si dimezzano a vicenda e rallentano anche le sincronizzazioni. Una richiesta rifiutata riceve `409` con il nome dell'esecuzione che occupa il posto
